...
```{r summarize the fits that are plotted below}
example_cohorts <- c(75,289,120)

example_cohorts_df <- tidy_fits|>
  filter(cohort_id %in% example_cohorts)|>
  mutate(example_name = case_when(cohort_id == 75 ~ "k_good",
                                  cohort_id == 289 ~ "leading",
                                    cohort_id == 120 ~ "lagging"))|>relocate(example_name)

example_cohorts_df|>
  select(example_name,cohort_id,discpar_a,discpar_k1,discpar_k2,weibull_alpha,weibull_beta,negexp_k)|>
  left_join(litter_df|>group_by(cohort_id)|>slice_head(n=1)|>select(Detritus_Species, Detritus_Condition, Geog_Locale, Mesh_Size_Category))
...
weibulls.p2 <- weibulls.p+theme(
  axis.text = element_text(size = 16, face = "bold"),
  axis.title = element_text(size = 18, face = "bold")
)
  
  ggsave(weibulls.p2, filename = "Weibull_spaghetti.jpeg", path = here("paper/figures"),
...
```{r alternative plot}
fit_counts.p_larger <- 
  fit_counts.p + theme(legend.text = element_text(size = 18), legend.title = element_text(size = 18),
                       axis.title = element_text(size = 18, face = "bold"), axis.text = element_text(size = 16, face = "bold"))+ylab("# of Time Series")

sl_aic.p_larger <- 
  sl_aic.p + theme(legend.text = element_text(size = 18), legend.title = element_text(size = 18),
                       axis.title = element_text(size = 18, face = "bold"), axis.text = element_text(size = 16, face = "bold"))+xlab("Number of Observations")+ylab("Negative Exponential Favored via AICc")
s1 <- cowplot::plot_grid(fit_counts.p_larger, sl_aic.p_larger,nrow = 2)

s2 <- cowplot::plot_grid(s1, weibulls.p2,nrow = 1)
ggsave(s2, filename = "BreakdownAintConstant.jpeg", path = here("paper/figures"),
...
k_termmass_brm3 <- brm(bf(negexp_k ~ terminal_mass_prop+terminal_meas_day),
                      family=lognormal(),
                      data = tidy_fits|>filter(weibull_alpha <20), 
                      chains = 4, iter = 2000, warmup = 500,
                      file = here("data/derived_data/regression_fits/k_termmass_brm3"))

k_termmass_brm3.1 <- brm(bf(negexp_k ~ terminal_mass_prop+terminal_meas_day+weibull_alpha),
                      family=lognormal(),
                      data = tidy_fits|>filter(weibull_alpha <20), 
                      chains = 4, iter = 2000, warmup = 500,
                      file = here("data/derived_data/regression_fits/k_termmass_brm3.1"))

k_termmass_brm3.2 <- brm(bf(negexp_k ~ terminal_mass_prop*terminal_meas_day+weibull_alpha),
                      family=lognormal(),
                      data = tidy_fits|>filter(weibull_alpha <20)|>mutate(logterm = log(terminal_mass_prop)), 
                      chains = 4, iter = 2000, warmup = 500,
                      file = here("data/derived_data/regression_fits/k_termmass_brm3.2"))
...
newdata_k_termmass2 <- expand_grid(weibull_alpha = c(0.1,0.5,0.75,1.2,3.8),
                        terminal_meas_day = seq(min(tidy_fits$terminal_meas_day), max(tidy_fits$terminal_meas_day), by = 5),
                        terminal_mass_prop = seq(min(tidy_fits$terminal_mass_prop), max(tidy_fits$terminal_mass_prop), by =0.05))
  
  mutate(logterminal_mass_prop = log(terminal_mass_prop),
         `logterminal_mass_prop:terminal_meas_day` = logterminal_mass_prop *terminal_meas_day)
                        
k_termmass_preds<-predictions(k_termmass_brm6,newdata = newdata_k_termmass)|>
  mutate(weibull_alpha = as_factor(weibull_alpha))

k_termmass_preds2<-avg_predictions(k_termmass_brm3.1, newdata = newdata_k_termmass2, variables = c("weibull_alpha", "terminal_mass_prop"))|>
  mutate(weibull_alpha = as_factor(weibull_alpha))
#plot_predictions(k_termmass_brm2, newdata = newdata_k_termmass,condition = c("terminal_mass_prop", "weibull_alpha"), points = 0.5)+
  k_termmass.p <- k_termmass_preds|>
    ggplot()+
  ...
  k_termmass.p2 <- k_termmass_preds2|>
    ggplot()+
  geom_ribbon(aes(x = terminal_mass_prop, ymax = conf.high, ymin = conf.low, fill = weibull_alpha), alpha = 0.2)+
  geom_line(aes(x = terminal_mass_prop, y = estimate,color = weibull_alpha))+
  geom_point(data = tidy_fits|>filter(weibull_alpha <20), aes(x = terminal_mass_prop, y = negexp_k), 
